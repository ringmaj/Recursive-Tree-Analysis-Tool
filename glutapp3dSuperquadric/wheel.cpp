...
void Wheel::createBox(double x0, double y0, double z0, double length, double width, double height, GsArray<GsVec> &P, GsArray<GsColor> &C, GsColor color) {
...
void Wheel::build(double nfaces, double rt, double rb, double len)
{
	...
	C.size(P.size()); C.setall(GsColor::white);
...
void Wheel::draw(const GsMat& tr, const GsMat& pr, const GsLight& l)
{
	float f[4];
